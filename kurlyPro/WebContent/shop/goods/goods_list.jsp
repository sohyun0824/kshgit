			...
			<div id="content">
				...
				<div class="page_aticle">
					<div id="lnbMenu" class="lnb_menu">
						...
						<div class="inner_lnb">
							<div class="ico_cate">
								<c:if test="${ category.length() le 2 }">
								<span class="ico" style="display: block;">
									<img src="../data/category/icon_on_${ categoryList[0].parent_seq }.png" id="goodsListIconView" alt="카테고리 아이콘">
								</span> 
								<span class="tit">${ categoryList[0].pc_name }</span>
								</c:if>
								 <c:if test="${ category.length() gt 2 }">
								 <span class="ico">
									 ...
									 <span class="tit">베스트</span>
									 </c:if>
									 <c:if test="${ category eq 'sale' }">
									 <span class="tit">알뜰쇼핑</span>
									 </c:if>
								 </c:if>
							</div>
							<ul class="list">
								<c:if test="${ category.length() le 2 }">
								<li><a href="goods_list.do?category=${ categoryList[0].parent_seq }">전체보기</a></li> 
								<c:forEach items="${ categoryList }" var="dto">
									<li><a href="goods_list.do?category=${ dto.child_seq }">${ dto.cc_name }</a></li>
								</c:forEach>
								<script>
									$("#lnbMenu .inner_lnb li a[href$='${ category }']").addClass("on");
								</script>
								</c:if>
								<c:if test="${ category.length() gt 2 }">
								<li><a href="goods_list.do?category=${ category }" class="on">전체보기</a></li>
								</c:if>
							</ul>							
						</div>
					</div>
					...
					<div id="goodsList" class="page_section section_goodslist">
							...
							<div class="sort_menu">
								<div class="">
									...
									<div class="select_type user_sort">
										...
										<ul class="list">
											<c:if test="${ category.length() le 2 }">
												<li><a href="goods_list.do?category=${ category }&option=recommend" class="">추천순</a></li>
											</c:if>
											<c:if test="${ category eq 'sale' }">
												<li><a href="goods_list.do?category=${ category }&option=sale" class="">혜택순</a></li>
											</c:if>
											<li><a href="goods_list.do?category=${ category }&option=new" class="">신상품순</a></li>
											<li><a href="goods_list.do?category=${ category }&option=hot" class="">인기상품순</a></li>
											<li><a href="goods_list.do?category=${ category }&option=low" class="">낮은 가격순</a></li>
											<li><a href="goods_list.do?category=${ category }&option=high" class="">높은 가격순</a></li>
										</ul>
									</div>
								</div>
							</div>
						...
						<script>
							$(".sort_menu .select_type").click(function() {
								$(this).toggleClass("checked");
							})
							
							$(".sort_menu .list a[href$='${ option }']").addClass("on");
							
							var txt = $(".sort_menu .list a[href$='${ option }']").text();
							$(".sort_menu .name_select").text(txt);
						</script>
								...
								<ul class="list">
									...
									<c:forEach items="${ goodsListView.goodsList }" var="dto">
										<li>
											<div class="item">
												<div class="thumb">
													<a href="goods_view.do?group_no=${ dto.group_no }" class="img" style="background-image: url(../..${ dto.main_img });">
						...
						<div class="layout-pagination">
							<div class="pagediv">
								<a href="goods_list.do?category=${ category }&page=1&option=${ option }" class="layout-pagination-button layout-pagination-first-page">맨 처음 페이지로 가기</a> 
								<a href="goods_list.do?category=${ category }&page=${ goodsListView.currentPage-1 == 0 ? 1 : goodsListView.currentPage-1 }&option=${ option }" class="layout-pagination-button layout-pagination-prev-page">이전 페이지로 가기</a>
								<c:forEach var="num" begin="1" end="${ goodsListView.pageTotal }" step="1">
									<span>
										...
										<c:if test="${ num ne goodsListView.currentPage }">
											<a class="layout-pagination-button layout-pagination-number" href="goods_list.do?category=${ category }&page=${ num }&option=${ option }">${ num }</a>
										</c:if>
									</span>
								</c:forEach>
								<a href="goods_list.do?category=${ category }&page=${ goodsListView.currentPage+1 > goodsListView.pageTotal ? goodsListView.pageTotal : goodsListView.currentPage+1 }&option=${ option }" class="layout-pagination-button layout-pagination-next-page">다음 페이지로 가기</a> 
								<a href="goods_list.do?category=${ category }&page=${ goodsListView.pageTotal }&option=${ option }" class="layout-pagination-button layout-pagination-last-page">맨 끝 페이지로 가기</a>
							</div>
						</div>
...
<script>
	...
	$("#cartPut .btn_type1 button").on("click", function() {
		if(!<%= login %>){
			...
			location.href="../member/login.do?return_url=/kurlyPro/shop/goods/goods_list.do?category=<%= request.getParameter("category") %>";
		} else{
			// 로그인O
